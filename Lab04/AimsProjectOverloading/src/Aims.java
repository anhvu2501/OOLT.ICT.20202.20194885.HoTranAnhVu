...
//        anOrder.addDigitalVideoDisc(dvd3);
        System.out.println("Enter the number of DVDs you want to add to cart?");
        int n;
        Scanner sc = new Scanner(System.in);
        n = Integer.parseInt(sc.nextLine());
        DigitalVideoDisc []listOfDisc = new DigitalVideoDisc[n];
        String title;
        String category;
        String director;
        int length;
        double cost;
        System.out.println("Enter DVDs' information: ");
        for (int i = 0; i < n; i ++) {
            title = sc.nextLine();
            category = sc.nextLine();
            director = sc.nextLine();
            length = Integer.parseInt(sc.nextLine());
            cost = Double.parseDouble(sc.nextLine());
            listOfDisc[i] = new DigitalVideoDisc(title, category, director, length, cost);
        }
        anOrder.addDigitalVideoDisc(listOfDisc);
        DigitalVideoDisc dvd1 = new DigitalVideoDisc("The Lion King", "Animation", "Roger Allers", 87, 19.95f);
        DigitalVideoDisc dvd2 = new DigitalVideoDisc("Star Wars", "Science Fiction", "George Lucas", 87, 24.95f);
